...
- **Disposition des tests webui (mypy racine unique)** : `packages/webui/tests/` est **PLAT** (aucun `__init__.py`), comme verifier/matching — un 2ᵉ paquet `tests` entrerait en collision avec celui du crawler sous le mypy racine. Donc : (a) **aucun import `from tests…`** ; les fixtures partagées (schéma DDL des bases) vivent dans `packages/webui/tests/conftest.py`, exposées comme **fixtures pytest** (`catalog_db`/`local_db` → `Path` d'une base temporaire au schéma DDL appliqué, vide ; chaque test insère ses lignes) ; (b) **chaque fichier de test est nommé `test_webui_<chose>.py`** (stems globalement uniques across verifier+matching+webui) ; (c) la garde « templates sans logique » vit dans `src/catalog_webui/_dev/check_templates.py` (importable comme `catalog_webui._dev.check_templates`, donc sous coverage), **pas** sous `scripts/`. (Ceci remplace, là où ils divergent, les mentions `tests/fixtures/schema.py` / `from tests.fixtures…` / `scripts/check_templates.py` des tâches ci-dessous.)
- **sqlfluff** ne lint que des **fichiers `.sql`** (les migrations du crawler). Le SQL de lecture du webui vit dans des **constantes Python** (exactement comme les requêtes du crawler, elles aussi non lintées par sqlfluff) → le gate sqlfluff reste `packages/crawler/src` seul ; **ne PAS** y ajouter `packages/webui/src` (pointer sqlfluff sur un dossier sans `.sql` ne lint rien — faux signal). La correction du SQL webui est garantie par les **tests** (100 % branch + cas « dernier par hash »), pas par sqlfluff.
...
> **Risque #1 (lecture WAL vivante)** : tranché à l'impl (Task 4 code + Task 13 montage) et documenté `agents/reference/` — ne PAS le relitiger en cours de route.
