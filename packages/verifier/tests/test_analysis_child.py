...
class _RecordingConfiner:
    """Spy Confiner: records whether it was called (proof of kernel-ring order/installation)."""
...
def test_clamav_signature_makes_egress_malicious(
    ...
    # healthy media BUT the injected clamav_runner matches a signature → malicious verdict.
    (tmp_path / _HASH).write_bytes(b"\x1a\x45\xdf\xa3" + b"\x00" * 64)
...
def test_vanished_file_prints_suspicious_egress(
    ...
    # canonical hash but the file is gone (vanished between is_file and spawn) → suspicious.
    code = main(
        [_HASH],
        ffprobe_runner=_StubFfprobe(0, _VALID_MEDIA),
        clamav_runner=_CLEAN_CLAMAV,
...
def test_read_header_rejects_non_regular_file(tmp_path: Path) -> None:
    # sandbox-confinement#4 regression (isolated S_ISREG False branch): an fd opened on a
    # non-regular type (here a directory — ``os.open`` succeeds on macOS + Linux) must raise
    # OSError. The branch is tested DIRECTLY on the helper to avoid depending on the behavior
    # of ``os.fdopen`` (which pre-rejects "Is a directory" on some OSes even before our check
    # runs).
...
def test_read_header_rejects_symlink(tmp_path: Path) -> None:
    # ``O_NOFOLLOW`` branch → ELOOP. Defense-in-depth exposed in isolation.
...
def test_read_header_reads_a_regular_file(tmp_path: Path) -> None:
    # Happy branch: regular file → bytes returned. Covers the normal exit.
...
def test_directory_in_quarantine_is_refused_with_suspicious_egress(
    ...
    # sandbox-confinement#4 regression (S_ISREG False branch): O_NOFOLLOW does NOT reject a
    # directory (which is not a symlink). The ``fstat + S_ISREG`` after open detects it and
    # returns ``suspicious`` (defense-in-depth against non-regular types: dir, FIFO, etc.).
    quarantine = tmp_path / "quarantine"
    quarantine.mkdir()
    (quarantine / _HASH).mkdir()  # a directory instead of a regular file
...
def test_symlink_in_quarantine_is_refused_with_suspicious_egress(
    ...
    # sandbox-confinement#4 regression: a symlink in place of the quarantine file must be
    # REFUSED by O_NOFOLLOW (open raises ELOOP → the except OSError → suspicious). Defense-in-
    # depth consistent with verify_file parent-side.
    target = tmp_path / "outside"
    target.write_bytes(b"\x1a\x45\xdf\xa3" + b"\x00" * 64)  # a real media header
    quarantine = tmp_path / "quarantine"
    quarantine.mkdir()
...
def test_only_header_bytes_are_read(tmp_path: Path) -> None:
    # a huge file: the child must read only header_bytes (not the whole file).
    big = tmp_path / _HASH
    ...
    # read only 8 bytes for the sniff (the test proves no memory crash)
    assert code == 0
...
def test_main_defaults_cfg_and_runners_without_real_subprocesses(
    ...
    # covers the `cfg is None`, `ffprobe_runner is None` AND `clamav_runner is None` branches
    # WITHOUT running real ffprobe/clamscan: monkeypatch from_env + both Prod*Runner (→ stubs).
...
def test_confiner_is_called_before_pipeline_run(
    ...
    # kernel ring: the confiner MUST be installed BEFORE entering pipeline.run (so before the
    # ffprobe_runner runs). An ffprobe stub that asserts `confiner.called` proves the order.
    (tmp_path / _HASH).write_bytes(b"\x1a\x45\xdf\xa3" + b"\x00" * 64)
    confiner = _RecordingConfiner()

    class _OrderAssertingFfprobe:
        def __call__(self, argv: Sequence[str]) -> tuple[int, bytes]:
            assert confiner.called, "the confiner must be installed BEFORE pipeline.run/ffprobe"
...
def test_confiner_not_called_when_file_missing(
    ...
    # missing file → the `except OSError` branch returns BEFORE the install point: the ring is
    # NOT installed (proof the install is indeed after the header read, not before).
    confiner = _RecordingConfiner()
    code = main(
        [_HASH],
        ffprobe_runner=_StubFfprobe(0, _VALID_MEDIA),
...
def test_seccomp_enabled_selects_prod_confiner(tmp_path: Path) -> None:
    # _default_confiner returns the TYPE per config — WITHOUT calling __call__ (otherwise a real
    # seccomp filter would install itself in the test process).
