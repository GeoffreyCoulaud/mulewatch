"""Entry point ``python -m emule_indexer``: loads the config, builds the app, runs (§4).
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="emule_indexer", description="eMule crawler (observer)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    # constraint (compose runs ``python -m emule_indexer --config … --targets …`` with no subcmd).
