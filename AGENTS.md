...
**EC / amuled (empirical — facts established by hardware probes / source reading, see `agents/reference/`):**
- *EC = External Connection, the TCP protocol through which the crawler commands the aMule daemon (`amuled`). Defined by aMule, opcodes documented in `agents/reference/ec-protocol.md`.*
