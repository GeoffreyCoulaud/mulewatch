...
- **SQLite in a Docker Desktop bind mount is not coherent between processes.** With `/data` on a
  ...
  the identical scenario passes, and so does the same scenario on the native engine with a real
  kernel bind mount. Reproduced on `a1c6def` too, so it predates this lot. The suite can now be
  pointed at another engine: `_run` forwards `DOCKER_CONTEXT` and `DOCKER_HOST` to the CLI and
  strips everything else, with the negative path under test (a socket nothing listens on must fail
  the call, which is what a stripped variable would silently turn into a success on the default
  daemon).
- **The native engine on this machine cannot create a veth pair**, so it runs containers only with
  `--network none` and compose cannot use it at all. The cause is not a kernel config
  (`CONFIG_VETH=m`) but a pending reboot: pacman upgraded the kernel and removed the running
  kernel's module tree, so no module that is not already resident can be loaded. Docker Desktop is
  unaffected because its VM boots its own kernel. The operator's to fix, by rebooting. This is what fails `test_a_file_amuled_shares_is_recorded_
...
- **The compose smoke's completion scenario under Docker Desktop.** `test_a_file_amuled_shares_
  is_recorded_completed` fails there, and it **already failed before this lot**: bisected on
  2026-09-22 by building `a1c6def` (the branch point, EC adapter still in place) in a worktree and
  running that one test against it, same machine, same bind-mounted `/data`. Identical failure,
  identical signature (`expected 'completed True', last was 'downloading False'`), and a
  hand-driven repro on that old image shows the crawler logging `✅ download completed` while
  another process reading the same file still sees `downloading` with `last_seen_at` frozen. The
  counter-experiment settles the cause: the same scene run on the **native** engine, with a real
  kernel bind mount of a host directory, completes and is visible both to a second process in the
  container and to the host. It is the Desktop VM's mediated mount, not the code, and not
  SQLite's fault either. The other three tests of that suite pass. Recorded for the reader in
  `docs/contributing/testing.md` §3.4.
