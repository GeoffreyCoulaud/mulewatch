...
**Current state:** only the **matching engine** is built — a complete, pure-domain pipeline (`src/emule_indexer/domain/`) that turns a YAML config + episode targets into a deterministic, explainable decision for any observed filename. The network/EC adapter, the SQLite data model, auto-download/verifier, observability, and packaging are **not built yet**. Milestones are tagged `v0.1.0-foundations` → `v0.4.0-engine` on `main` (not pushed).
