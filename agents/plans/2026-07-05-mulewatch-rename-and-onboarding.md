...
**Spec:** `agents/specs/2026-07-05-mulewatch-rename-and-onboarding-docs.md` (decisions D1-D11).
...
- Dated historical docs (`agents/handoffs/`, `agents/specs/`, `agents/plans/`, `agents/reference/`)
...
- [ ] **Step 4.2**: Review the diff by eye: entrypoint commands must read
  ...
  `agents/handoffs|specs|plans|reference`).
...
- [ ] **Step 9.1**: Rewrite in French per spec phase 3, exact section order:
  ...
  6. `## Pour les développeurs` : stack line, `uv run poe check`, links to
     `agents/specs/2026-06-10-crawler-mvp-design.md`, `docs/testing-guide.md`, `agents/plans/`.
...
- [ ] **Step 10.1**: Rewrite in French. Royal road: numbered steps, each ending with
  ...
  - *Annexe B. Activer le mode download* : `download.enabled: true` in
    ...
    `agents/reference/2026-06-17-amuled-completion-behavior.md`; transient behaviors kept
...
- [ ] **Step 13.3**: Handoff `agents/handoffs/2026-07-05 - handoff - mulewatch rename + onboarding docs.md`
