...
`agents/reference/2026-06-13-ec-download-opcodes.md` : rapport calqué sur `2026-06-11-ec-field-richness.md` (convention de fiabilité SOURCE / EMPIRIQUE / PENDING). Contenu MINIMAL à rédiger :
...
```bash
git add src/emule_indexer/tools/download_probe.py tests/tools/test_download_probe.py pyproject.toml tests/integration/test_amuled_download.py "agents/reference/2026-06-13-ec-download-opcodes.md"
git commit -m "$(cat <<'EOF'
...
```
Expected: `1 passed` (mécaniques EC réelles : add_link + download_queue contre un amuled réel). Coller la sortie dans `agents/reference/2026-06-13-ec-download-opcodes.md` (section EMPIRIQUE).
...
Créer `agents/handoffs/2026-06-13 - handoff - download capability.md` (format des handoffs précédents) :
...
```bash
git add "agents/handoffs/2026-06-13 - handoff - download capability.md"
git commit -m "$(cat <<'EOF'
...
| §4 Port `MuleDownloadClient` + extension EC adapter, probe documenté | Tasks 4, 5, 6 (rapport `agents/reference/2026-06-13-ec-download-opcodes.md`) |
